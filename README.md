...
`<SECRET_PATH>` should be your path to your `client_secret.json` file containing the token for your Client ID (see Building & Running section for more details)
`<DISK_TOKEN_PATH>` should be your path to a `tokencache.json` or some similarly named json file. This is needed to prevent any redundant authentication on your Gmail Account.

- `trash [NUM_THREADS] <SECRET_PATH> <DISK_TOKEN_PATH> {by-labels|by-msg-ids|by-filter}`: allows user to trash all emails in specific gmail label(s), a series of message IDs, or with a query filter
...
- `labels <SECRET_PATH> <DISK_TOKEN_PATH>`: allows user to see all labels within their gmail
...
- `filter [NUM_THREADS] <SECRET_PATH> <DISK_TOKEN_PATH> <query through options|json file with query|txt file with query>`: allows user to query a search on their gmail inbox and receive an email blurbs related to the query within desired txt file
